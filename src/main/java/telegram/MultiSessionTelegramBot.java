...
public class MultiSessionTelegramBot extends TelegramLongPollingBot {
    ...
    public Message sendTextMessage(Long chatId, String text) {
        long underscoreCount = text != null ? text.chars().filter(c -> c == '_').count() : 0;
        if (underscoreCount % 2 == 0) {
            SendMessage command = createApiSendMessageCommand(chatId.toString(), text);
            return executeTelegramApiMethod(command);
        } else {
            var message = "Строка '%s' является невалидной с точки зрения markdown. Воспользуйтесь методом sendHtmlMessage()".formatted(text);
            System.out.println(message);
            return sendHtmlMessage(chatId, message);
        }
    }
    ...
    public Message sendHtmlMessage(Long chatId, String text) {
        SendMessage message = new SendMessage();
    ...
    public void sendPhotoMessage(Long chatId, InputFile photo, String caption) {
        ...
        sendPhoto.setParseMode("HTML"); // Если курс придёт с разметкой
        try {
            execute(sendPhoto);
    ...
    private SendMessage createApiSendMessageCommand(String chatId, String text) {
        SendMessage message = new SendMessage();
        message.setChatId(chatId);
        message.setText(text);
        message.setParseMode("markdown"); // если ты используешь Markdown
        return message;
    }
